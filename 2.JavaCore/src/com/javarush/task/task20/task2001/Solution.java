...
public class Solution {
    ...
    public static class Human {
        ...
        public void save(OutputStream outputStream) throws Exception {
            ...
            if (isAssets) {
                oos.writeBoolean(true);
                oos.writeInt(assets.size());
                for (int i = 0; i < assets.size(); i++) {
                    oos.writeObject(assets.get(i).getName());
                    oos.writeDouble(assets.get(i).getPrice());
                }
        ...
        public void load(InputStream inputStream) throws Exception {
            ...
            boolean isName = ois.readBoolean();

            if (isName) {
                name = (String) ois.readObject();
            }

            boolean isAssets = ois.readBoolean();

            if (isAssets) {
                int sizeArray = ois.readInt();
                for (int i = 0; i < sizeArray; i++) {
                    assets.add(new Asset((String) ois.readObject(), ois.readDouble()));
                }
            }
